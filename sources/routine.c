...
/*   Updated: 2024/11/19 06:53:17 by ecastong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	eat(t_philo *philo)
{
	...
	philo->time_last_eaten = log_msg(philo, 0, ID_EAT);
	philo->times_eaten++;
	safe_mutex(philo->info_lock, pthread_mutex_unlock);
...
static void	recursive_loop(t_philo *philo)
{
	...
	if (extract_state(*philo) == STOPPED)
		return ;
	log_msg(philo, 0, ID_SLEEP);
	wait_ms(philo->params.time_to_sleep);
	if (extract_state(*philo) == STOPPED)
		return ;
	log_msg(philo, 0, ID_THINK);
	if (extract_state(*philo) == STOPPED)
		return ;
